...
test('interpretação local de BRL sem cálculos de decisão',()=>{
 ...
 assert.equal(localIntent('Queria gastar 20 reais').amountCents,2000);
 assert.equal(localIntent('Gostaria de gastar uns R$ 35,90 hoje').amountCents,3590);
 assert.equal(localIntent('Eu poderia gastar 80 reais?').amountCents,8000);
