...
export async function freeConversation(message:string,snapshot:Snapshot,history:History,request:typeof fetch=fetch,rawPreferences:unknown=DEFAULT_PREFERENCES){
 ...
 const spend=async(amount:number)=>{const assessment=assess(snapshot,amount),facts=assessmentFacts(assessment),fallback=personalityFallback(assessment,preferences,message);return {mode:onlineMode,reply:await styled(facts,fallback,message,history,preferences,request),assessment};};
