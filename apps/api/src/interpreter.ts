...
export function localIntent(message:string):Intent {
  ...
  const m=message.trim().match(/^(?:(?:eu\s+)?(?:quero|queria|posso|poderia)\s+gastar|(?:eu\s+)?gostaria\s+de\s+gastar|gastar)\s+(?:(?:uns?|cerca\s+de)\s+)?(?:R\$\s*)?(\d+(?:\.\d{3})*(?:,\d{1,2})?)(?:\s+(?:real|reais))?(?:\s+hoje)?\s*[?!.]?$/i);
