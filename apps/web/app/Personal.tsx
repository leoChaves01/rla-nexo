...
import {addMonths,salaryOccurrence,salaryParts} from '@rla-nexo/engine';
...
export default function Personal({initial,onSaved}:{initial:Data;onSaved:()=>void}){
 ...
 const [entry,setEntry]=useState({accountId:initial.snapshot.accounts[0]?.id||'',kind:'expense',amountCents:0,description:'',date:initial.snapshot.asOf,commitmentId:undefined as string|undefined,incomeId:undefined as string|undefined,salaryPartId:undefined as string|undefined,salaryReferenceMonth:undefined as string|undefined,salary:false});
 function edit(fn:(s:Snapshot)=>void){setDraft(s=>{const next=structuredClone(s);fn(next);if(next.salary?.parts)next.salary.netCents=next.salary.parts.reduce((n,p)=>n+p.amountCents,0);return next});setNotice('');setFeedback({});}
 ...
 function prepare(values:Partial<typeof entry>){
  ...
  setEntry({accountId:draft.accounts[0]?.id||'',kind:'expense',amountCents:0,description:'',date:draft.asOf,commitmentId:undefined,incomeId:undefined,salaryPartId:undefined as string|undefined,salaryReferenceMonth:undefined as string|undefined,salary:false,...values});
 ...
 return <div className="personal">
  ...
  <fieldset disabled={busy}>
   ...
   <form className="panel salary-settings" onSubmit={e=>saveSection(e,'salary')}><div className="section-title"><h2>2. Meu salário</h2>{!draft.salary?<button type="button" className="secondary" onClick={()=>edit(s=>{s.salary={netCents:0,payday:Number(s.asOf.slice(-2)),accountId:s.accounts[0]?.id||'',active:true,confirmed:true,occurrences:[]}})}>Cadastrar salário</button>:<button type="button" className="text-button" onClick={()=>edit(s=>{s.salary=null})}>Desativar salário</button>}</div>
   <p>Cadastre uma vez. O Nexo gera cada recebimento mensal automaticamente e guarda apenas as confirmações.</p>
   {draft.salary&&<>{!draft.accounts.length&&<div className="pending-note" role="status"><p>Salve a regra agora; a conta só é necessária para confirmar a entrada e atualizar o saldo.</p><button type="button" className="secondary" onClick={()=>{edit(s=>{s.accounts.push({id:id(),name:'',balanceCents:0});});requestAnimationFrame(()=>document.getElementById('minhas-contas')?.scrollIntoView({behavior:'smooth'}))}}>Adicionar conta de destino</button></div>}<p>Total mensal previsto: <strong>{money(draft.salary.netCents)}</strong>.</p>
   {!draft.salary.parts?<><div className="edit-row salary-fields"><Money label="Salário líquido mensal (R$)" value={draft.salary.netCents} onChange={v=>edit(s=>{s.salary!.netCents=v})}/><label>Todo dia<input type="number" min={1} max={31} required value={draft.salary.payday??1} onChange={e=>edit(s=>{s.salary!.payday=Number(e.target.value)})}/></label></div><button type="button" className="secondary" onClick={()=>edit(s=>{const sal=s.salary!,payday=sal.payday??1;sal.parts=[{id:id(),amountCents:Math.floor(sal.netCents/2),payday},{id:id(),amountCents:sal.netCents-Math.floor(sal.netCents/2),payday}]})}>Dividir salário em partes</button></>:<><p>As partes se repetem mensalmente. Alterações passam a valer para os próximos recebimentos.</p>{draft.salary.parts.map((part,i)=>{const occurrence=salaryOccurrence(draft,part,draft.asOf.slice(0,7));const label=occurrence.status==='RECEIVED'?'✓ Recebido neste mês':occurrence.status==='OVERDUE'?'Aguardando confirmação':'Previsto para '+new Date(occurrence.scheduledDate+'T12:00:00').toLocaleDateString('pt-BR');return <div className="edit-row" key={part.id}><strong>Parte {i+1}<small>{label}</small></strong><Money label={'Valor da parte '+(i+1)+' (R$)'} value={part.amountCents} onChange={v=>edit(s=>{s.salary!.parts![i].amountCents=v})}/><label>Todo dia<input type="number" min={1} max={31} required value={part.payday} onChange={e=>edit(s=>{s.salary!.parts![i].payday=Number(e.target.value)})}/></label>{occurrence.status==='RECEIVED'?null:occurrence.scheduledDate>draft.asOf?<span className="muted">{label}</span>:<button type="button" className="secondary" onClick={()=>prepare({kind:'income',salary:true,salaryPartId:part.id,salaryReferenceMonth:occurrence.referenceMonth,amountCents:part.amountCents,accountId:draft.salary!.accountId,description:'Salário · parte '+(i+1)})}>Confirmar recebimento</button>}<button type="button" className="text-button" disabled={draft.salary!.parts!.length<=1} onClick={()=>edit(s=>{s.salary!.parts!.splice(i,1)})}>Remover parte</button></div>})}<button type="button" className="secondary" disabled={draft.salary.parts.length>=12} onClick={()=>edit(s=>{s.salary!.parts!.push({id:id(),amountCents:0,payday:Number(s.asOf.slice(-2))})})}>Adicionar parte do salário</button></>}
   <label>Conta de destino (opcional)<select value={draft.salary.accountId} onChange={e=>edit(s=>{s.salary!.accountId=e.target.value})}><option value="">Definir depois</option>{draft.accounts.map(a=><option key={a.id} value={a.id}>{a.name||'Conta sem nome'}</option>)}</select></label><label className="check"><input type="checkbox" checked={draft.salary.active??draft.salary.confirmed??true} onChange={e=>edit(s=>{s.salary!.active=e.target.checked;s.salary!.confirmed=e.target.checked})}/>Incluir a renda recorrente na projeção</label>
   {!draft.salary.parts&&(()=>{const part=salaryParts(draft)[0],occurrence=salaryOccurrence(draft,part,draft.asOf.slice(0,7));return occurrence.status==='RECEIVED'?<p className="saved-message">✓ Recebido neste mês. Próximo previsto para o mês seguinte.</p>:occurrence.scheduledDate>draft.asOf?<p className="muted">Previsto para {new Date(occurrence.scheduledDate+'T12:00:00').toLocaleDateString('pt-BR')}.</p>:<div className="entry-actions"><button type="button" className="secondary" onClick={()=>prepare({kind:'income',salary:true,salaryPartId:part.id,salaryReferenceMonth:occurrence.referenceMonth,amountCents:part.amountCents,accountId:draft.salary!.accountId,description:'Salário recebido'})}>Confirmar recebimento</button></div>})()}<p>Confirmar atualiza o saldo real uma única vez. A próxima previsão é criada automaticamente.</p></>}
